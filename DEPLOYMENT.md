...
- The `/tmp/tmp.*` pattern is generated by mktemp in the deployment workflow for temporary files
- The wildcard in journalctl commands allows any number of log lines; monitor access logs if concerned about abuse
- Regularly review and audit sudo privileges with `sudo cat /etc/sudoers.d/pdfmerger-deploy`
- Consider using a dedicated deployment user with limited system access
- For enhanced security, you can replace wildcards with more specific patterns:
  ```bash
  # Example: Limit journalctl to specific line counts
  pdfmerger ALL=(ALL) NOPASSWD: /bin/journalctl -u pdfmerger -n [0-9]*
  pdfmerger ALL=(ALL) NOPASSWD: /bin/journalctl -u pdfmerger -n [1-9][0-9]*
  ```
